...
(() => {
  ...
  if (typeof structuredClone === "undefined") {
    let structuredClonePolyfill = function (value, _options) {
      const seen = new Map();
      const clone = (v) => {
        ...
        if (v instanceof Date) {
          const c2 = new Date(v.getTime());
          seen.set(v, c2);
          return c2;
        }
        if (v instanceof RegExp) {
          const c2 = new RegExp(v.source, v.flags);
          seen.set(v, c2);
          return c2;
        }
        if (v instanceof Map) {
          const c2 = new Map();
          seen.set(v, c2);
          for (const [k, val] of v) c2.set(clone(k), clone(val));
          return c2;
        }
        if (v instanceof Set) {
          const c2 = new Set();
          seen.set(v, c2);
          for (const val of v) c2.add(clone(val));
          return c2;
        }
        ...
        if (ArrayBuffer.isView(v)) {
          ...
          const c2 = new ctor(buf, v.byteOffset, v.length);
          seen.set(v, c2);
          return c2;
        }
        if (Array.isArray(v)) {
